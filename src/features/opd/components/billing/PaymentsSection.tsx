...
import { useOpdBillPayments, useRecordOpdPayment } from "../../hooks";
import { PAYMENT_MODE_LABEL } from "../../../ipd/constants";
import { IPD_PAYMENT_MODES } from "../../../../types/ipd";
import type { BillPaymentMode } from "../../../../types/ipd";
import type { OPDBill } from "../../../../types/opd";

export function PaymentsSection({ bill, visitId }: { bill: OPDBill; visitId: number }) {
  const toast = useToast();
  const payments = useOpdBillPayments(bill.id);
  const recordPayment = useRecordOpdPayment(visitId);
      ...
      {balance > 0 ? (
        recording ? (
          <View className="gap-3 rounded-lg border border-border bg-card p-3">
            ...
            />
            <Button title="Record payment" size="sm" onPress={submit} loading={recordPayment.isPending} />
          </View>
        ) : (
