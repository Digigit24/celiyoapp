...
import { useCreateBillItem, useDeleteBillItem, useUpdateBillItem } from "../../hooks";
import { BILL_ITEM_SOURCE_COLOR } from "../../constants";
import { IPD_SOURCE_OPTIONS } from "../../../../types/ipd";
import type { BillItemSource, CatalogType, IPDBillItem } from "../../../../types/ipd";
import { CatalogSearchField, type CatalogOption } from "./CatalogSearchField";
...
function BillItemRow({
  ...
  const updateItem = useUpdateBillItem(admissionId);
  const deleteItem = useDeleteBillItem(admissionId);
      ...
      <View className="flex-1">
        <View className="flex-row items-center gap-1.5">
          <View
            ...
            style={{ backgroundColor: `${BILL_ITEM_SOURCE_COLOR[item.source]}22` }}
          >
            <Text className="text-[10px] font-medium" style={{ color: BILL_ITEM_SOURCE_COLOR[item.source] }}>
              {item.source}
            </Text>
          </View>
...
function AddItemForm({ billId, admissionId, onDone }: { billId: number; admissionId: number; onDone: () => void }) {
  const toast = useToast();
  const createItem = useCreateBillItem(admissionId);
    ...
    <View className="gap-3 rounded-lg border border-border bg-card p-3 mt-2">
      ...
      {tab === "custom" ? (
        <>
          ...
          <Select
            label="Source"
            options={IPD_SOURCE_OPTIONS.map((s) => ({ label: s, value: s }))}
            value={source}
...
export function BillItemsSection({
  ...
  return (
    <View>
      {items.map((item) => (
        <BillItemRow key={item.id} item={item} admissionId={admissionId} locked={locked} />
      ))}
      {items.length === 0 ? <Text className="text-sm text-muted-foreground py-3">No items yet.</Text> : null}

      {!locked ? (
        adding ? (
          <AddItemForm billId={billId} admissionId={admissionId} onDone={() => setAdding(false)} />
        ) : (
          <Pressable
