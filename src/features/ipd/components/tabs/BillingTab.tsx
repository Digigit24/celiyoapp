...
import { useBills, useCreateBill } from "../../hooks";
...
export function BillingTab({ admissionId }: { admissionId: number }) {
  const toast = useToast();
  const bills = useBills(admissionId);
  const createBill = useCreateBill(admissionId);
  ...
  return (
    <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 32 }}>
      ...
      {selected ? <BillDetailCard bill={selected} admissionId={admissionId} /> : null}
    </ScrollView>
  );
}
